...
public:
    ...
    template <class Other> Array(Array <Other> const & obj);

    Array& operator=(const Array& obj);
    template <class Other> Array& operator=(Array <Other> const& other);
...
{
    this->size = size;
    // конструктора по умолчанию не существует, поэтому память выделяем через char(1 байт * Размер T) и 
...
inline Array<T>::Array(Array <Other> const& obj)
{
    size = obj.getSize();
    array = (T*) new char[size * sizeof(T)];
    for (size_t i = 0; i != size; ++i)
...
template <class T>
template <class Other> 
inline Array<T>& Array<T>::operator=(Array <Other> const& other)
{
